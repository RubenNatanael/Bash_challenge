...
function calculate_total_bill() {
    ...
    counter=1
    while IFS=', ' read -r product2 pricee; do
        if [ -n "${order[$counter]}" ] && [ "${order[$counter]}" -gt 0 ]; then
            price_int=$((pricee))
            
            if [[ $price_int =~ ^[0-9]+$ ]]; then
                total=$(( $total + ${order[$counter]} * price_int ))
            fi
            #echo "${order[$counter]}"
        fi
        counter=$(( counter + 1))
    done < <(tr -d '\r' < "menu.txt")
    echo $total
}
...
read -p "What is your name?" customer_name
